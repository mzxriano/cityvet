...
<div class="min-h-screen">
  <main class="p-6 max-w-4xl mx-auto">
    ...
    <div class="flex items-center space-x-4 mb-6">
      ...
      <h1 class="text-2xl font-semibold text-primary">Archive Record</h1>
    </div>

    <!-- Archive Header -->
    <div class="bg-gradient-to-r from-gray-600 to-gray-500 rounded-lg p-6 text-white mb-6">
      <div class="text-center">
        ...
        <p class="text-sm opacity-75 mt-2">Archived: {{ \Carbon\Carbon::parse($archive->created_at)->format('F j, Y') }}</p>
      </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-lg p-6 mb-6">
      <h3 class="text-lg font-semibold text-gray-800 mb-4">Deletion Reason</h3>
    ...
    @if($archive->notes)
    <div class="bg-white rounded-lg shadow-lg p-6 mb-6">
      <h3 class="text-lg font-semibold text-gray-800 mb-4">Archive Notes</h3>
      ...
      <div class="space-y-3">
        ...
        <div class="flex justify-between">
          ...
          <span class="inline-flex items-center px-2 py-1 bg-gray-100 text-gray-800 rounded-full text-sm">
            <i class="fas fa-archive mr-1"></i> Deleted
          </span>
        </div>
      </div>
    </div>
    ...
    <!-- Restore Section -->
    <div class="bg-white rounded-lg shadow-lg p-6">
      <h3 class="text-lg font-semibold text-gray-800 mb-4">Restore Animal</h3>
      <p class="text-gray-600 mb-4">
        This animal was deleted but can be restored. Restoring will make the animal active again and remove it from the archives.
      </p>
      <form method="POST" action="{{ route('admin.archives.restore', $archive->id) }}" 
            onsubmit="return confirm('Are you sure you want to restore {{ $archive->animal->name }}? This will make the animal active again and remove it from archives.')">
        @csrf
        <button type="submit" 
                class="bg-green-600 hover:bg-green-700 text-white font-medium py-2 px-4 rounded-lg transition-colors">
          <i class="fas fa-undo mr-2"></i>Restore Animal
        </button>
      </form>
    </div>
  </main>
