            ...
            <div class="flex flex-col sm:flex-row gap-3 sm:gap-4">
                @if(request('search'))
                    <a href="{{ route('admin.barangay') }}" 
                       class="bg-gray-500 text-white px-4 py-2.5 rounded-lg hover:bg-gray-600 transition text-sm text-center font-medium shadow-sm">
                        Clear Search
                    </a>
                @endif

                @if(request('search'))
                    <div class="flex items-center gap-2 text-sm text-gray-600 bg-blue-50 px-3 py-2 rounded-lg border border-blue-200">
                        <svg class="w-4 h-4 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                        </svg>
                        <span>Results for: <strong>"{{ request('search') }}"</strong></span>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                            ...
                            <div class="grid grid-cols-3 gap-3">
                                <div class="bg-white rounded-lg p-3 text-center border border-pink-200">
                                    <div class="text-xl font-bold text-pink-600">{{ $barangay->pet_owners_count }}</div>
                                    <div class="text-sm text-gray-600 mt-1">Pet</div>
                                </div>
                                <div class="bg-white rounded-lg p-3 text-center border border-orange-200">
                                    <div class="text-xl font-bold text-orange-600">{{ $barangay->livestock_owners_count }}</div>
                                    <div class="text-sm text-gray-600 mt-1">Livestock</div>
                                </div>
                                <div class="bg-white rounded-lg p-3 text-center border border-yellow-200">
                                    <div class="text-xl font-bold text-yellow-600">{{ $barangay->poultry_owners_count }}</div>
                                    <div class="text-sm text-gray-600 mt-1">Poultry</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                            ...
                            <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
                                <div class="bg-white rounded-lg p-3 text-center border border-green-200">
                                    <div class="text-lg font-bold text-green-600">{{ $barangay->activities->where('category', 'vaccination')->count() }}</div>
                                    <div class="text-sm text-gray-600 mt-1">Vaccination</div>
                                </div>
                                <div class="bg-white rounded-lg p-3 text-center border border-purple-200">
                                    <div class="text-lg font-bold text-purple-600">{{ $barangay->activities->where('category', 'deworming')->count() }}</div>
                                    <div class="text-sm text-gray-600 mt-1">Deworming</div>
                                </div>
                                <div class="bg-white rounded-lg p-3 text-center border border-indigo-200">
                        ...
                        <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                            ...
                            <div class="grid grid-cols-3 gap-3">
                                <div class="bg-white rounded-lg p-3 text-center border border-pink-200">
                                    <div class="text-xl font-bold text-pink-600">{{ $barangay->vaccinated_pets_count ?? 0 }}</div>
                                    <div class="text-sm text-gray-600 mt-1">Pet</div>
                                </div>
                                <div class="bg-white rounded-lg p-3 text-center border border-orange-200">
                                    <div class="text-xl font-bold text-orange-600">{{ $barangay->vaccinated_livestock_count ?? 0 }}</div>
                                    <div class="text-sm text-gray-600 mt-1">Livestock</div>
                                </div>
                                <div class="bg-white rounded-lg p-3 text-center border border-yellow-200">
                                    <div class="text-xl font-bold text-yellow-600">{{ $barangay->vaccinated_poultry_count ?? 0 }}</div>
                                    <div class="text-sm text-gray-600 mt-1">Poultry</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                            ...
                            <div class="bg-white rounded-lg p-4 text-center border border-red-200">
                                ...
                                <div class="text-sm text-gray-600 mt-1">Total Reports</div>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <tr class="bg-gray-100 border-t border-gray-300">
                            ...
                            <th class="px-4 py-3 border-r border-gray-300">
                                <div class="grid grid-cols-3 gap-2 text-xs font-medium">
                                    ...
                                    <div class="text-center bg-yellow-100 text-yellow-700 rounded-md py-2 border border-yellow-300">
                                        Poultry
                                    </div>
                                </div>
                            </th>
        ...
        @if($barangays->hasPages())
            <div class="mt-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between pt-6 border-t border-gray-200">
                <div class="text-sm text-gray-700 text-center sm:text-left bg-gray-50 px-4 py-2 rounded-lg border border-gray-200">
                    ...
                    Showing <span class="font-semibold">{{ $barangays->firstItem() }}</span> to <span class="font-semibold">{{ $barangays->lastItem() }}</span> of <span class="font-semibold">{{ $barangays->total() }}</span> results
                </div>
