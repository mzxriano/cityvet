...
<div x-data="reportsData()">
    ...
    <!-- Bite Case Reports Tab -->
    <div x-show="activeTab === 'bite-case'" class="w-full bg-white rounded-xl p-[2rem] shadow-md overflow-x-auto">
        <!-- Header Actions -->
        <div class="mb-4 flex justify-between items-center">
            <div class="flex items-center gap-4">
                <h3 class="text-lg font-semibold text-gray-700">Confirmed Bite Case Reports</h3>
            </div>
            
            <div class="flex gap-2">
                <!-- Generate Excel Button -->
                <form method="POST" action="{{ route('reports.generate-bite-case-excel') }}" class="inline">
                    @csrf
                    <!-- Include current filters -->
                    @if(request('bite_species'))
                        <input type="hidden" name="bite_species" value="{{ request('bite_species') }}">
                    @endif
                    @if(request('bite_provocation'))
                        <input type="hidden" name="bite_provocation" value="{{ request('bite_provocation') }}">
                    @endif
                    @if(request('bite_date_from'))
                        <input type="hidden" name="bite_date_from" value="{{ request('bite_date_from') }}">
                    @endif
                    @if(request('bite_date_to'))
                        <input type="hidden" name="bite_date_to" value="{{ request('bite_date_to') }}">
                    @endif
                ...
                <input type="text"
                       ...
                       placeholder="Search bite case reports..."
        ...
        <form method="GET" action="{{ route('admin.reports') }}" class="mb-6 bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
                ...
                <div>
                    ...
                    <select name="bite_species" class="w-full border border-gray-300 px-3 py-2 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500"
                            onchange="this.form.submit()">
                        <option value="">All Species</option>
                        @foreach($biteSpecies as $species)
                            <option value="{{ $species }}" {{ request('bite_species') == $species ? 'selected' : '' }}>
                                {{ ucfirst($species) }}
                            </option>
                        @endforeach
                    </select>
                ...
                <!-- Provocation Filter -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Bite Provocation</label>
                    <select name="bite_provocation" class="w-full border border-gray-300 px-3 py-2 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500"
                            onchange="this.form.submit()">
                        <option value="">All Provocations</option>
                        @foreach($biteProvocations as $provocation)
                            <option value="{{ $provocation }}" {{ request('bite_provocation') == $provocation ? 'selected' : '' }}>
                                {{ ucfirst($provocation) }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="date" 
                           name="bite_date_from" 
                           value="{{ request('bite_date_from') }}"
                           class="w-full border border-gray-300 px-3 py-2 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500"
                           onchange="this.form.submit()">
                </div>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Records Per Page</label>
                    <select name="bite_per_page" class="w-full border border-gray-300 px-3 py-2 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500"
                            onchange="this.form.submit()">
                        <option value="10" {{ request('bite_per_page', 10) == 10 ? 'selected' : '' }}>10</option>
                        <option value="25" {{ request('bite_per_page') == 25 ? 'selected' : '' }}>25</option>
                        <option value="50" {{ request('bite_per_page') == 50 ? 'selected' : '' }}>50</option>
                        <option value="100" {{ request('bite_per_page') == 100 ? 'selected' : '' }}>100</option>
                        <option value="all" {{ request('bite_per_page') == 'all' ? 'selected' : '' }}>All</option>
                    </select>
                </div>

                <!-- Clear Filters Button -->
                <div class="flex items-end">
                    <a href="{{ route('admin.reports') }}?tab=bite-case" 
                       class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-md transition w-full text-center">
                        Clear All Filters
                    </a>
                </div>
            ...
            <!-- Active Filters Display -->
            @if(request()->hasAny(['bite_species', 'bite_provocation', 'bite_date_from', 'bite_date_to', 'bite_per_page']))
                <div class="mt-4 flex flex-wrap gap-2">
                    <span class="text-sm font-medium text-gray-700">Active filters:</span>
                    @if(request('bite_species'))
                        <span class="bg-red-100 text-red-800 px-2 py-1 rounded-full text-xs">
                            Species: {{ ucfirst(request('bite_species')) }}
                        </span>
                    @endif
                    @if(request('bite_provocation'))
                        <span class="bg-orange-100 text-orange-800 px-2 py-1 rounded-full text-xs">
                            Provocation: {{ ucfirst(request('bite_provocation')) }}
                        </span>
                    @endif
                    @if(request('bite_date_from'))
                        <span class="bg-indigo-100 text-indigo-800 px-2 py-1 rounded-full text-xs">
                            From: {{ \Carbon\Carbon::parse(request('bite_date_from'))->format('M d, Y') }}
                        </span>
                    @endif
                    @if(request('bite_date_to'))
                        <span class="bg-indigo-100 text-indigo-800 px-2 py-1 rounded-full text-xs">
                            To: {{ \Carbon\Carbon::parse(request('bite_date_to'))->format('M d, Y') }}
                        </span>
                    @endif
                    @if(request('bite_per_page') && request('bite_per_page') != '10')
                        <span class="bg-gray-100 text-gray-800 px-2 py-1 rounded-full text-xs">
                            Per page: {{ request('bite_per_page') }}
                        </span>
                    @endif
                </div>
            @endif
        ...
        <table class="table-auto w-full border-collapse">
            <thead class="bg-[#d9d9d9] text-left text-[#3D3B3B]">
                <tr>
                    ...
                    <th class="px-4 py-2 font-medium">Animal Species</th>
                    <th class="px-4 py-2 font-medium">Bite Provocation</th>
                    <th class="px-4 py-2 font-medium">Location</th>
                    <th class="px-4 py-2 font-medium">Incident Date</th>
                    <th class="px-4 py-2 font-medium">Confirmed By</th>
                    <th class="px-4 py-2 font-medium rounded-tr-xl">Confirmed Date</th>
                </tr>
            </thead>
            <tbody>
                @forelse($biteCaseReports as $index => $report)
                    <tr class="hover:bg-gray-50 border-t text-[#524F4F]"
                        x-show="!biteCaseSearch || 
                                '{{ strtolower($report->victim_name) }}'.includes(biteCaseSearch.toLowerCase()) ||
                                '{{ strtolower($report->species) }}'.includes(biteCaseSearch.toLowerCase()) ||
                                '{{ strtolower($report->bite_provocation) }}'.includes(biteCaseSearch.toLowerCase()) ||
                                '{{ strtolower($report->location_address) }}'.includes(biteCaseSearch.toLowerCase()) ||
                                '{{ strtolower($report->confirmed_by ?? '') }}'.includes(biteCaseSearch.toLowerCase())">
                        <td class="px-4 py-2">{{ $biteCaseReports->firstItem() + $loop->index }}</td>
                        <td class="px-4 py-2">{{ $report->victim_name }}</td>
                        <td class="px-4 py-2">{{ $report->age }}</td>
                        <td class="px-4 py-2">
                            <span class="px-2 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">
                                {{ ucfirst($report->species) }}
                            </span>
                        </td>
                        <td class="px-4 py-2">
                            <span class="px-2 py-1 rounded-full text-xs font-medium 
                                {{ $report->bite_provocation === 'provoked' ? 'bg-orange-100 text-orange-800' : 'bg-red-100 text-red-800' }}">
                                {{ ucfirst($report->bite_provocation) }}
                            </span>
                        </td>
                        <td class="px-4 py-2">{{ Str::limit($report->location_address, 30) }}</td>
                        <td class="px-4 py-2">{{ \Carbon\Carbon::parse($report->incident_time)->format('M d, Y H:i') }}</td>
                        <td class="px-4 py-2">{{ $report->confirmed_by ?? 'N/A' }}</td>
                        <td class="px-4 py-2">
                            {{ $report->confirmed_at ? \Carbon\Carbon::parse($report->confirmed_at)->format('M d, Y') : 'N/A' }}
                        </td>
                    </tr>
                @empty
                    <tr>
